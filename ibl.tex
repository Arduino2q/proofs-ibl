...
Thus we may write 
the set of the primes less than ten as either
$\set{2, 3, 5, 7}$ or~$\setbuilder{p\in\N}{\text{$p<10$ and $p$ is prime}}$ 
...
\remark as we did in the paragraph following the definition, 
we often blur the distinction between a function and its graph; for instance 
we might say, ``a function is an input-output relationship'' when really
the pairing is the function's graph.
