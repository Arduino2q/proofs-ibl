...
By inspection it is  reflexive and
symmetric.
...
and
(i)~$\sequence{0,1},\sequence{1,1}\in R_5$ and~$\sequence{1,1}\in R_5$.
...
$M=\setbuilder{(a,b)\in\Z^2}{a\equiv b\pmod m}$  
is an equivalence.
\begin{ans}
Exercise~\ref{ex:AEquivBpmodMIFFABDifferBYMultipleOfM}
shows that for $m>0$ the statement $a\equiv b\pmod m$
is equivalent to the statement that the two differ by a multiple of~$m$
$m\divides (a-b)$.
