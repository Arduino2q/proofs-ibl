...
\item $X=\Z$, $\partition{P}=\setbuilder{P_n}{n\in\Z}$
      ...
      (\hint some pairs $i,j$ give $P_i=P_j$ and so 
      what appears to be two elements of~$\partition{P}$ 
      may in fact be only one element.) 
...
\item No $p_n$ is empty because $n\in p_n$, as~$n\equiv n\pmod 3$.
  The observation that $n\equiv n\pmod 3$ 
  also shows that every~$n\in\N$ is an element of at least
  one~$p_n$.
  ...
  containment, start with~$y\in p_i$, so $y\equiv i\pmod 3$.
  Exercise~\ref{ex:EquivModMIsEquivalenceRelation}
  shows that the relation of `equivalent modulo~$3$' is an 
  equivalence relation.
  Symmetry implies that $i\equiv x\pmod 3$, and together with
  $y\equiv i\pmod 3$
  transitivity implies that $y\equiv x\pmod 3$.
  Apply transitivity again, using that $x\equiv j\pmod 3$, to conclude that
  $y\equiv j\pmod 3$, which means that~$y\in p_j$.
  Containment in the other direction is similar.  

  Finally, $P$ has three elements: $p_0=p_3=p_{-3}=\cdots\hbox{}$, and
  $p_1=p_4=p_{-2}=\cdots{}$, and~$p_2=p_5=p_{-1}=\cdots\hbox{}$.
\item Observe that $x\in p_x$ so no~$p_x$ is empty.
  That same observation shows that every $x\in\R$ is an element of some~$p$.
