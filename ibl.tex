...
we show that the statement holds for some first number;
...
prove that our example statement is true for all~$n\geq 1$.
...
\begin{ex}
The sequence of \definend{Fibonacci numbers} is
...
\begin{ex} Decide and justify each:
\begin{items}
...
\begin{center} \small
  ...
  \end{tabular}
\end{center}
We use the bits $0$ and~$1$ instead of $F$ and~$T$ so that the 
row is the binary representation of the row number.
...
(The distinction is there because the codomain is not determined
...
\item $X=\R$, $\partition{P}=\setbuilder{P_x}{x\in \R}$
      ...
      \hspace{1em}\hint some pairs $x,y\in\R$ give $P_x=P_y$
      and so what may appear to be two elements of~$\partition{P}$
      could in fact be a single element.
\item $X=\Z$, $\partition{P}=\setbuilder{P_n}{n\in\Z}$
      where $P_n=\setbuilder{i\in\Z}{i\equiv n\pmod 3}$\hspace{1em}%
      \hint some pairs $i,j$ give $P_i=P_j$. 
\end{exes}
\begin{ans}
\begin{exes}
...
\item If $f$ is onto then for any $c\in C$ there is a~$d\in D$
  such that $f(d)=c$.
  So one of the equivalence classes that is an element of~$P$ is the set of 
  $d\in D$ such that $f(d)=c$.
  Then $\hat{f}$ of that class is $c$.
\end{exes}
\end{ans}
\end{ex}
