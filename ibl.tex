...
(restated, the pair $(p,q)\\Z\times\Z^+$\ is related to $(n,d)$
if $pd=qn$).
List five elements.
...
\begin{ans}
\begin{exes}
\item We can express the lines using analytic geometry, 
  fixing an $xy$-axis, and giving the equations.
  Parallel lines have the same slope so
  five pairs are $(y=2x,y=2x+1)$, $(y=x,y=x-3)$, $(y=4x,y=4x)$
  $(y=-3,y=-5)$, and~$(y=(3/2)x+1,y=(3/2)x-1)$.    
\item As in the prior item, we can express the lines using analytic geometry, 
  fixing an $xy$-axis, and giving the equations.
