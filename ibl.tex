...
% \begin{ex}
% If $D$ and~$C$ are finite sets and there is a correspondence~$\map{f}{D}{C}$
% then the two sets have the same number of elements.  
% \begin{ans}
% We will do induction on the numbe of elements in~$D$ to prove 
% that if there is a correspondence $\map{f}{D}{C}$ then
% $|C|=|D|$.
% In the $|D|=0$ base case, the correspondence is a trivial 
% (its graph is the empty set) and because this function is onto the
% set~$C$ must be empty also, with $|C|=0$.

% For the inductive step assume that the statement is true for $|D|=0$, 
% \ldots, $|D|=k$ and consider the $|D|=k+1$~case.
% To get a set of size~$k$,
% put an element of~$D$ aside by picking some $d\in D$ and considering
% $\hat{D}=D-\set{d}$.
...
% $\restrictionmap{f}{\hat{D}}(x_0)=\restrictionmap{f}{\hat{D}}(x_1)$
% then $f(x_0)=f(x_1)$ and so $x_0=x_1$.

% So the inductive hypothesis applies and we have that 
% $\range(\restrictionmap{f}{\hat{D}})$ has $k$-many elements.
% Consider~$f(d)$.
% Because $f$ is one-to-one, $f(d)\notin\range(\restrictionmap{f}{\hat{D}})$
% and thus $\range(f)$ has one more element than 
% $\range(\restrictionmap{f}{\hat{D}})$.
% Because $f$ is onto, $\range(f)=N$.
% Thus $|C|=k+1=|D|$.
% \end{ans}
% \end{ex}
...
then if $(x,y)\in R$ we say $x$ is~\definend{$R$-related} to~$y$,
...
\textit{For any $x$, symmetry is that $x$ is related to~$y$ 
...
Multiply both sides of the first equation by $ab$ to get $pdab=qnab$, reqroup as
...
\item The relation is reflexive since two lines that are the same are
  ...
  The relation is symmmetric since if $\ell_0$ is related to~$\ell_1$
...
\begin{df}
A \definend{partition}~$P$ of a set $X$ is a 
collection of nonempty subsets of~$X$ such that every element $x\in X$ 
is in exactly one of these subsets.
That is, $P$ partitions~$X$ if and only if 
$P$'s elements are nonempty,
the union of $P$'s elements is equal to~$X$ 
($P$~\definend{covers}~$X$),
and $P$'s elements are \definend{pairwise disjoint}
($p_i\intersection p_j=\emptyset$ for $i\neq j$).
\end{df}
...
\begin{ex} Verify that the~$P$ is a partition of the~$X$.  
  How many elements are in~$P$?
\begin{exes}
\item $X=\N$, $P=\set{p_0,p_1}$ where~$p_0$ is the set of even numbers
  and $p_1$ is the set of odd numbers
\item $X=\Z$, $P=\setbuilder{p_i}{i\in\Z}$
      where $p_n=\setbuilder{i\in\Z}{i\equiv n\pmod 3}$\hspace{1em}%
      (\hint for some pairs $i,j$ we have $p_i=p_j$ and so 
      what may look like two elements of~$P$ is in fact only one element.) 
\item $X=\R$, $P=\setbuilder{p_x}{x\in \R}$
      where $p_x=\setbuilder{y\in\R}{x-y\in Z}$
\end{exes}
\begin{ans}
\begin{exes}
\item To verify that it is a partition we will check the three conditions.
  First, that no element $p_i\in P$ is empty is clear.
  Second, that every element of~$\N$ is an element of some~$p_i$ is
  also clear.
  The third is also clear; no element of~$\N$ is both even and odd.

  Finally, $P$ has two elements, since $p_0\neq p_1$.
\item To see that no $p_n$ is empty observe that $n\equiv n\pmod 3$ and
  so $n\in p_n$.
  The same observation shows that every $n\in\N$ is an element of at least
  one~$p_n$.

  To see that the $p_i$'s are pairwise disjoint, first observe that 
  for some pairs $i,j\in\Z$ the sets $p_i$ and~$p_j$ are equal
  (an example is that $p_1=p_4$).
  So we must show that if the sets are unequal then they are
  disjoint: if $p_i\neq p_j$ then $p_i\intersection p_j=\emptyset$.

  We will prove the equivalent implication that if 
  $p_i\intersection p_j\neq \emptyset$ then $p_i=p_j$
  so assume that $x\in (p_i\intersection p_j)$.
  Then $x\equiv i\pmod 3$ and~$x\equiv j\pmod 3$ so that by 
  Exercise \ref{ex:AEquivBpmodMIFFABDifferBYMultipleOfM} we have 
  $3\divides (x-i)$ and~$3\divides (x-j)$,
  and thus $x-i=3\cdot k_i$ and $x-j=3\cdot k_j$ for some $k_i,k_j\in Z$.
  We want to show mutual containment between the sets $p_i$ and~$p_j$.
  so let $y$ be an element of~$p_i$,
  Then $y\equiv p_i\pmod 3$, so $3\divides (y-i)$,
  and so $y-i=3\cdot k_y$ for some~$k_y\in\Z$.
  Subtracting gives~$(y-i)-(x-i)=y-x=3\cdot(k_y-k_i)$,
  so $y\equiv x\pmod 3$ and therefore  
...
\item Show that the set of inverse images 
  $P=\setbuilder{f^{-1}(c)}{c\in C}$ form a partition of the domain.
\item Consider the map
  $\map{\hat{f}}{P}{C}$ whose action is given by
  $\hat{f}(p)$ is $f(d)$ for any~$d\in p$.
  Show that~$\hat{f}$ is a function, and that it is a correspondence.
\end{exes}
\end{ex}
