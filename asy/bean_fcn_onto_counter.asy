...
pair one=bean0+(0.18*r,0.40*r);
pair two=bean0+(0.14*r,0.14*r);
dot("{$0$}",zero,align=W);
dot("{$1$}",one,align=W);
dot("{$2$}",two,align=W);
...
path one_to_b = one{dir(20)}..b;
path two_to_c = two{dir(20)}..c;
...
path one_to_b_drawn = subpath(one_to_b,0.04,0.96);  
draw(one_to_b_drawn,black,BeginBar(bar_length),Arrow(TeXHead),margin=NoMargin);
path two_to_c_drawn = subpath(two_to_c,0.04,0.96);  
draw(two_to_c_drawn,black,BeginBar(bar_length),Arrow(TeXHead),margin=NoMargin);

path a_to_alpha = a{dir(30)}..alpha;
