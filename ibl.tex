...
while starting with a month of preliminary material might be a problem.
In my course classes begin with some students going to the 
board to write their answer to the homework, and I use the five or 
ten minutes to show slides that address the issues that 
the class's current work raises, such as the truth table 
definition of the implication operator.
In this way those topics are covered, but the main emphasis lies on 
the class's work proving things.  
...
Sometimes say map or morphism.
Alternative notation $D\rightarrow^f C$.

Maps-to notation.
Argument and value.

Two functions are equal only if they have the same domain, codomain,
and graph.
...
\begin{df}
A function is \definend{one-to-one} if for each value there is at most
one associated argument, that is, if $f(d_0)=f(d_1)$ implies that $d_0=d_1$
for elements $d_0,d_1$ of the domain.
A function is \definend{onto} if for each value there is at least
...
\definend{correspondence} or \definend{permutation}.     
\end{df}
