...
\item Before we start we bserve that there are $x_0,x_1\in\R$ such that
...
\item  As in the prior item we will define a map
  ...
  Jackie Chan, when up against multiple bad guys, alternates amoung his 
  attackers.
...
\item We will show that no $\map{f}{\N}{\powerset(\N)}$ is onto.
  ...
  because then $i$~satifies the defining criteria for membership in~$R$, namely
