...
\begin{center}\small
  ...
  \hspace*{2em}
  \vcenteredhbox{if $a\cong b$ then $A\cong B$}
\end{center}
...
Does it not require an argument?
...
Here are examples of assertions that seem convincing 
...
\end{center}
However, that pattern breaks down;  
for $n=41$ the output $41^2+41+41$ is clearly 
divisible by~$41$.
...
\begin{center} \small\smallskip
  ...
  \end{tabular}}\smallskip
\end{center}
P\`olya conjectured that below any~$n>1$ the even types do not
outnumber the odd types. 
The numerical evidence is strong\Dash 
the statement holds until $906\,150\,257$\Dash 
but that number gives a counterexample. 
\end{itemize}
\end{frame}
% >>> def prime_factors(n):
% ...     i = 2
...
\end{center}
\pause
In this context `not~$P$' is symbolized $\bar{P}$.
Note that $\bar{P}=1-P$.
...
\begin{frame}
  ...
  No matter how hard the propositional logic sentence, with patience 
...
\pause  Use $n=6$ to get that $F\rightarrow T$ must evaluate to $T$.  
\pause Use $n=3$ to get that $F\rightarrow F$ should yield $T$.
