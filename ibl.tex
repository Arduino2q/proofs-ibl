...
\item One of the Divisibility Properties in \ref{ex:DivisibilityProperties}
  is that for any nonzero integer~$a$ the largest divisor is $\absval{a}$.
  Since $a$ is positive, $\absval{a}=a$.
...
is the smallest number that is a multiple of each.
\end{df}
...
\begin{ex}
For each of these pairs $a,b$ compute $\gcd(a,b)$, $\lcm(a,b)$, $ab$, and
...
\begin{ex}
\begin{exes}
\item Prove that any common multiple of two numbers is divisible by the 
  least common multiple.
  \hint apply the Division-Remainder theorem with the divisor $\lcm(a,b)$.
\item Prove that the product of two numbers is divisible by their 
  least common multiple.
\item For any numbers $a,b\in\Z$ let $d$ be given by $ab=\lcm(a,b)\cdot d$.
...
% \item Any common divisor of two numbers divides the greatest common divisor.
%   \hint in the case that both numbers $a$ and~$b$ are nonzero, 
%   consider their product and apply the prior item to get that 
%   it is a multiple of the least common multiple.
%   Write $ab=\lcm(a,b)\cdot s$ and show that $\absval{s}$ equals the
%   greatest common divisor by showing it is both greater than and less than
%   $\gcd(a,b)$. 
\end{exes}  
\begin{ans}  %% TODO check the zero and negative cases.
\begin{exes}
\item Suppose that $\lcm(a,b)=\ell$ and that $m$ is a common multiple
  of the two numbers.
  Dividing $m$ by~$\ell$ gives a quotient and remainder so that 
  $m=\ell\cdot q+r$ with $0\leq r< \ell$.
  We have that $r=m-\ell q$, and each term on the right is a multiple
  of both $a$ and~$b$.
...
\item Given $a,b\in\Z$, the product $ab$ is a common multiple of the two.
  Apply the prior item.
\item By the previous item, the least common multiple of $a,b\in\Z$ divides
  the product~$ab$.
  So $d$ is an integer for all number pairs $a,b$.
  
  Let~$c$ be a common divisor so that $c=a\cdot k_a$ and $c=b\cdot k_b$.
  ...
  as desired.
\item By the prior item, any common divisor of $a$ and~$b$ will divide~$d$.
  One such common divisor is $\gcd(a,b)$.
  This implies $\gcd(a,b)\leq\absval{d}$.
\item
  After the previous item, we need only show that $\gcd(a,b)\geq\absval{s}$ 
  Because $\lcm(a,b)$ is a common multiple of the two, 
  both $\ell/a$ and~$\ell/b$ are integers. 
  Reqriting the equation~$ab=\ell g$ to 
  $a=(\ell/b)\cdot d$ shows that~$d$ is a divisor of~$a$, and similarly
  it is a divisor of~$b$.
  Because $d$ is a common divisor, $\absval{d}\leq \gcd(a,b)$.
\item This is immediate from the prior items.
\item This is immediate from the prior items.
% \item 
%   If both $a$ and~$b$ are zero then the greatest common divisor is zero,
%   and any number divides zero.
%   If one number is zero and the other is nonzero then
%   without loss of generality we can take $a=0$ and~$b\neq 0$.
%   Since all numbers divide~$a$, the condition that a number~$c$ 
%   is a common divisor
...
%   The final case is that both numbers are nonzero.
%   Consider their product $ab$.
%   It is a common multiple of the two so the least common multiple 
%   $\ell=\lcm(a,b)$ is a factor; suppose that $ab=\ell\cdot s$.
%   We will show that $\absval{s}$ is the greatest common divisor.

%   We first show that any common divisor of $a$ and~$b$ also divides~$s$.
%   Let~$c$ be a common divisor so that $c=a\cdot k_a$ and $c=b\cdot k_b$
%   The number $ck_ak_b$ is a common multiple of $a$ and~$b$. 
%   By the prior item $\ell\divides ck_ak_b$.
%   Multiplication by $cs$ gives $\ell\cdot cs\divides ck_ak_b\cdot cs$.
%   Regroup to $(\ell s)c\divides (ck_a)(ck_b)s$, which is
%   $ab\cdot c\divides ab\cdot s$, and therefore by cancellation $c\divides s$,
%   as desired.
%   In particular, note that because $\gcd(a,b)$ divides both $a$ and~$b$, 
%   it divides~$s$, which implies that $\gcd(a,b)\leq\absval{s}$.
  
%   For the inequality in the other direction, 
%   start with the equation~$ab=\ell s$ and note that because $\ell$ is 
%   a common multiple, both $\ell/a$ and~$\ell/b$ are integers.
%   Thus $a=(\ell/b)\cdot s$ shows that~$s$ is a divisor of~$a$, and similarly
%   it is a divisor of~$b$.  
%   Therefore~$\absval{s}\leq \gcd(a,b)$.
\end{exes}
\end{ans}
\end{ex}
...
\begin{exes}
\item
  Let $\gcd(a,b)$ be~$d$ and suppose that $\gcd(a/d,\,b/d)=k$.
  Then $k\cdot j_a=a/d$ and~$k\cdot j_b=b/d$ for some numbers $j_a, j_b\in\Z$.
  Thus $kd\cdot j_a=a$ and $kd\cdot j_b=b$ and so $kd$ is a common divisor
  of $a$ and~$b$. 
  But $d=\gcd(a,b)$ and so $k=1$, or else $kd$ would be a common divisor
  that is larger than the greatest common divisor.    
\item
  Since $a\divides bc$ we know that $a$ is nonzero.
  If $b$ is zero then because $\gcd(a,b)=1$ we have that $\absval{a}=1$ 
  and $a\divides c$.
  In the $b\neq 0$ case, the assumptin that $\gcd(a,b)=1$ 
  along with the equation $\absval{ab}=\gcd(a,b)\cdot\lcm(a,b)$ shows that 
  $\lcm(a,b)=\absval{ab}$.
  ...
  Cancellation shows that $a\divides c$. 
\end{exes}
\end{ans}
\end{ex}
...
% We can perform Euclid's algorithm in reverse.
...
Otherwise it is \definend{composite}.  
\end{df}
...
\begin{ex} \notetext{Euclid's Lemma} Suppose that $p$ is a prime.  Prove each.
\begin{exes}
