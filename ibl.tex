...
\begin{ans}
One counterexample is to take $1,3\in\N$, since
$1-3$ is not defined on the natural numbers.  
To patch the statement change the $\N$ to~$\Z$:~where $a,b\in\Z$, 
the number $a+b$ is even if and only if $a-b$ is even.

For a proof of the patched statement, suppose that if $a,b\in\Z$.
...
\begin{ans}
\begin{exes}
\item For any number $a\in\Z$, we have $a=a\cdot 1$.
\item For every number $a\in\Z$ the equation~$0=a\cdot 0=0\cdot a$ holds.
  This shows that every number divides $0$ because it has $0$ as a 
  multiple of~$a$.
  It also shows that the only number that~$0$ divides is itself 
  because it shows that $0$ times any number is~$0$. 
