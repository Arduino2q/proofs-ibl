...
\item To see that no $p_n$ is empty observe that $n\equiv n\pmod 3$ and
  ...
  To see that the $p_i$'s are pairwise disjoint, observe that 
  ...
  Exercise \ref{ex:AEquivBpmodMIFFABDifferBYMultipleOfM} we have that
  $3\divides (x-i)$ and~$3\divides (x-j)$,
  so $x-i=3\cdot k_i$ and $x-j=3\cdot k_j$ for some $k_i,k_j\in Z$.
  Aiming to show the sets are mutually contained suppose that $y\in p_i$,
  so $y\equiv p_i\pmod 3$, so $3\divides (y-i)$,
  and so $y-i=3\cdot k_y$.
  % Adding gives $$
