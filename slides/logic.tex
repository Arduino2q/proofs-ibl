...
We say it is a \alert{unary logical operator} or call it 
...
between two sub-propositions is true if either of the two halves, or both,
is true.
...
List the action of the operators with a \alert{truth table}. 
\begin{center}
  \begin{tabular}[t]{c|c}
    $P$  &$\neg P$  \\ \hline
    $F$  &$T$  \\
...
an English sentence, for instance
$(P\vee Q)\wedge \neg(P\wedge Q)$ makes a hard to parse sentence.
\end{frame}
...
%..........
\begin{frame}
Sometimes using $0$ for~$F$ and $1$ for~$T$ is convenient.
On the left side of each table the rows make the ascending numbers written in
binary.
\begin{center}
  \begin{tabular}[t]{c|c}
    $P$  &$\bar{P}$  \\ \hline
...
The table makes clear why $P$~\text{and}~$Q$ is symbolized with a 
multiplication dot~$P\cdot Q$.

For $P$~\textit{or}~$Q$ the plus sign is the right symbol because 
...
``Let me go or the girl gets it'' all mean `one or the other, but not both'.
\begin{center}
  \begin{tabular}{cc|c}
...
We can lists all of the two-input logical operators.
\begin{center} \small
    \begin{tabular}{cc|c}
...
\pause
\bigskip
These are the one-input boolean functions.
\begin{center} \small
  \begin{tabular}{cccc}
    \begin{tabular}{c|c}
       $P$  &$\beta_0 P$  \\ \hline
...
Under our definition of implies both these statements are true, 
the first because its antecedent is false 
and the second because its consequent is true.
Many people say this violates their intuition and ask why we 
adopt this definition. 

\pause
Standard mathematical practice defines implication to make this
\begin{center}
  if $n$ is a perfect square then $n$ is not prime
\end{center}
come out true for all $n\in\N$.  
Take $x=6$ to get that $F\rightarrow T$ evaluates to $T$.  
Take $x=3$ to get that $F\rightarrow F$ gives $T$.
\end{frame}
...
Consider `if $n$ is odd then $n$ is a pefect square'.
It is an implication.
It also involves two parts, `$n$ is odd' and `$n$ is square',
whose truth value varies depending on the value of the variable.
In addition, there is an assertion that is not explicitely stated that
this implication should hold for all numbers.

A \alert{predicate} is a truth-valued statement with variables. 
Technically we can take them to be functions with truth values.
For instance, part of the example above would have $\text{Odd}(5)=T$.

A \alert{quantifier} describes the quantity of values for the variables
required to make the statement true.
There are two main quantifiers: `for all' denoted by $\forall$,
and `there exists' denoted $\exists$.
In the sentence above the `for all' is only implicitly stated.
\end{frame}
\begin{frame}
Examples of formally-translated statements.
